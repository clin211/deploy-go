众所周知，Vue SPA 单页面应用对 SEO 不友好，当然也有相应的解决方案。 **服务端渲染(SSR)** 就是常用的一种。 SSR 有利于 **搜索引擎优化(SEO, Search Engine Optimization)** ，并且 **内容到达时间(time - to - content)** （或称之为 **首屏渲染时长**）也有很大的优化空间。
